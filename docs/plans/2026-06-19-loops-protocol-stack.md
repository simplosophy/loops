...
> 日期：2026-06-19
> 状态：已确认（设计稿），待参考实现
> 范围：将 loops 从"软件实现"重新定位为"协议栈规范"，并填补生态缺失的人机协作协议
...
| L2 人机协作 | ❓ | agent ↔ 人 在项目/组织级协作 | ⛔ **生态空白** |
...
| 层 | 协议名 | 全称 | 定位 |
|----|--------|------|------|
| **L2** | **HACP** | Human–Agent Collaboration Protocol | loops 新建 — 人机协作 |
...
**"我们把已有协议分层归位，并补上了缺失的人机协作层"**——这是真正没人做、也最该做的事。
...
- **不做**：不定义 agent 内部怎么跑（那是 runtime 的事）、不定义 agent 怎么和人对话（那是 HACP）、不定义 agent 怎么调工具（那是 CAP）。
...
- **不做**：不定义 agent 内部执行、不定义 agent 间委派（那是 AAP）、不管 UI 渲染（HACP 是协议，不是前端规范）。
...
|------|---------|--------|---------|---------|
| **Task→Run 映射** | Task | L2 (HACP) | L2→L1 | Task 在 L1 触发一个 agent delegate；TaskID 必须贯穿到 AAP 的 delegate 调用 |
| **Checkpoint→Block 映射** | Checkpoint | L2 (HACP) | L2↔L1 | Checkpoint 挂起 Task 时，AAP 层对应的 agent delegate 必须进入 blocked 状态等待 |
...
- **依赖只能向下**：HACP 可调 AAP，AAP 可调 CAP；反向禁止。人永远不直接调 CAP，agent 永不被 HACP 反向控制以外的途径 push。
- **跨层只走契约对象**：Task、Checkpoint、CapabilityRef 是仅有的合法跨层载体。任何层不得直接读写邻层内部状态。
- **状态归属清晰**：Task 状态归 HACP，Run/agent 状态归 AAP，能力实例归 CAP。
...
- ❌ **Session/Conversation**：会话是 loop1（交互层）的事，HACP 只关心 Task 级协作。一个 Task 可以跨多个 session，一个 session 也可以包含多个 Task。
- ❌ **Permission/Role**：权限模型太组织相关，HACP 只声明 ownership，具体 RBAC 留给实现。
- ❌ **Notification**：通知是 channel（loop1）的事，HACP 只定义 Checkpoint 事件，怎么送达是 UI 的事。
...
HACP 的 API 面很小——这是刻意的。每个对象只有最小必要操作，遵循 loops 的极简原则。所有操作命名遵循 `<object>.<verb>`。
...
**操作总数：21 个**。对比 MCP（~15）、A2A（~10），HACP 略多但完全合理——因为它要治理的对象也更多（7 个一等对象）。
...
3. HACP 和 AAP（L1）的衔接点是 `delegate/resume`，和 loop1 channel 的衔接点是"通知送达"——层间契约干净。
...
| # | 议题 | 倾向 | 为什么留开放 |
|---|------|------|------------|
| 1 | **Transport 绑定**：HACP 是只定义语义、还是也绑定 HTTP/gRPC/WebSocket？ | 只定义语义，transport 留给实现（同 MCP 哲学） | 协议应 transport-agnostic，先有语义参考实现再收敛 |
...
| 6 | **Artifact 跨 project 引用**：A 项目的 Task 能否引用 B 项目的 Artifact？ | 允许，但需显式跨域授权 | 涉及权限模型，HACP 刻意不定义 RBAC |
| 7 | **版本兼容**：HACP v1 的 Task 能否被 v2 的 agent 执行？ | 语义版本 + 向后兼容保证 | 需要至少一次真实演进才能定规则 |
...
- **既有 `loop0/loop1/loop2` 软件分层**：仍然有效，它现在被理解为**协议栈各层的参考实现**。loop0 实现 CAP+AAP 的 host，loop1 实现 HACP 的 channel 适配，loop2 实现 HACP 的协议核心。
- **`docs/architecture/LOOPS_STACK.md`**：需要更新，把定位从"三层软件架构"调整为"三层协议栈规范"。
- **既有协议相关代码**（provider adapter、shell tool 等）：归位为 CAP 的参考实现，无需重写。
- **后续路线**：loops 的开发重心从"写 loop0/loop1/loop2 软件"转向"定义 HACP 规范 + 提供参考实现"。
...
2. **HACP 参考实现** — 用最小代码实现 21 个操作，验证协议可落地
3. **HACP spec 文档** — 从本设计稿提炼出独立的、对外发布的协议规范（类似 MCP spec）
