...
- `Checkpoint`：agent 或 harness 需要人决策时形成的阻塞点（含人发起的 interrupt）。
- `Artifact` / `Review`：交付物及人的验收记录（Review 分 plan / deliverable 两类）。
- `Ledger` / `Audit`：可累积、可重放、可审计的状态和事件。
- 连续控制值对象（0.2.0）：`steering_log`（append-only 方向修正）、
  `PermissionGrant` + `autonomy`（预授权能力边界与自主权档位）、
  `Checkpoint.proposed_actions`（批量审批与部分批准）。
- `HLPClient` / `HLPHost`：应用嵌入 HLP 的 SDK facade。
- `AgentAdapter`：HLP 向外部 harness 下发 delegate/block/resume/steer/handoff/cancel。
  `steer`（0.2.0）对应 `task.amend`，注入方向修正而不重启 run。
...
- `checkpoint.raise`（agent 发起）与 `task.interrupt`（人发起）必须对应 harness 的 block/pause 语义。
- `checkpoint.resolve` 必须把人的决策传回 harness；若携带 `state_patch` / `edited_artifact_ref`，harness 必须从该状态恢复而非原样 resume。
- `task.amend` 必须对应 harness 的 `steer`（注入方向修正，不重启 run）。
- `steering_log` 与 `PermissionGrant` 必须 append-only，不可删改（forward-only）。
