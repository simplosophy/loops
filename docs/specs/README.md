...
> HLP 是本项目的核心协议；L1 / L0 只作为接入既有 agent 与 capability 生态的路由说明。
...
Loops 当前聚焦 **Human Loop Protocol (HLP)**：定义人和自主 agent 围绕一个有边界的 Task 如何完成委派、把关、评审、交付、状态沉淀和审计。

已有生态已经覆盖两类能力：

- agent ↔ agent：A2A、ACP、AGNTCY 风格 mesh、各类自研 agent runtime
...
| [AAP.md](./AAP.md) | **L1 路由说明** | 说明 HLP 如何接入 A2A、ACP、AGNTCY 风格 mesh 或自研 agent runtime。不是新协议规范。 |
...
### 我要把现有 agent runtime 接到 HLP

读 [AAP.md](./AAP.md)。重点确认：

- HLP `Task.id` 如何进入 run correlation
- checkpoint 如何阻塞 / 恢复 run
- ownership transfer 如何映射到 runtime handoff
- runtime 事件如何保留审计所需的 task correlation
...
| **Ownership→Handoff** | Ownership ↔ Run | ownership 转移到新 agent 时，runtime handoff 必须保持原 TaskID。 |
