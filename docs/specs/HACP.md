...
| **规范版本** | 0.1.0-draft |
| **状态** | Draft — 等待参考实现验证 |
| **所属栈** | [Loops Protocol Stack](../plans/2026-06-19-loops-protocol-stack.md)（AI 协作的 OSI 模型） |
| **层级** | L2（栈顶层） |
...
HACP（Human-Agent Collaboration Protocol，人机协作协议）定义**人**与**自主 agent** 如何围绕一个有边界的工作单元（**Task**）进行委派、把关、交付与治理。它是 [Loops Protocol Stack](../plans/2026-06-19-loops-protocol-stack.md) 的顶层协议，填补了 MCP（agent↔工具）和 A2A（agent↔agent）未覆盖的维度：**agent 与其负责人之间的协作语义**。

HACP 是 transport-agnostic 的语义规范。本规范定义"协议说什么"，不规定"用什么线缆说"。
...
HACP 管辖：人↔agent 围绕 Task 的协作（委派、审批、交付、评审、状态沉淀、审计）。

HACP **不**管辖：
- agent 内部如何执行（实现自由）
- agent ↔ agent 委派（由 AAP / A2A 定义）
- agent ↔ 工具调用（由 CAP / MCP 定义）
- UI 渲染与通知送达（HACP 产出事件，channel 由 loop1 实现）
- 组织级 RBAC（HACP 只声明 ownership，权限模型留给实现）
...
HACP 定义 7 个一等对象。实现 **MUST** 支持全部 7 个。
...
HACP 是**只前进协议**（forward-only protocol）：
...
## 5. 层间契约（Inter-Layer Contracts）

HACP 是 Loops 栈的 L2。跨层通信 **MUST** 通过以下契约对象，**MUST NOT** 直接读写邻层内部状态。

### 5.1 HACP → AAP（L2 → L1）

| HACP 事件 | AAP 动作 | 契约 |
|----------|---------|------|
| `task.assign` | AAP `delegate` | TaskID **MUST** 贯穿到 AAP delegate 调用作为 correlation id |
| `checkpoint.raise` | AAP `block` | 对应 agent run **MUST** 进入 blocked 状态 |
| `checkpoint.resolve` | AAP `resume` | 对应 agent run **MUST** 恢复执行 |
| `ownership.delegate` | AAP `delegate`（子 agent） | 同 task.assign，但 parent 可追 |

### 5.2 HACP/CAP → Channel（→ loop1）

HACP 只产出事件，不负责送达。以下事件 **MAY** 被转译为 channel 通知：

| HACP 事件 | 典型 channel 表现 |
|----------|------------------|
| `checkpoint.raise` | 推送一条审批卡片到 IM |
| `artifact.commit + task→review_ready` | 推送 review 邀请 |
| `task.completed` | 推送完成通知 |

具体 UI/通知实现不属于 HACP 规范范围。

### 5.3 CapabilityRef（跨 L0/L1/L2）

上层引用能力时 **MUST** 只用 `(capability_id, version)`，**MUST NOT** 感知底层 transport（MCP stdio/SSE、Skills runtime）。
...
### 7.1 Transport 绑定
HACP 只定义语义。HTTP/gRPC/WebSocket mapping 留给实现，待参考实现后收敛。
...
### 7.7 版本兼容
HACP v1 的 Task 能否被 v2 的 agent 执行？语义版本 + 向后兼容的具体规则待演进验证。

---

## 8. 一致性级别（Conformance）

一个实现声称"符合 HACP 0.1.0-draft"，**MUST**：
...
人 Alice                     HACP 协议层                  agent Devin
  ...
  │── task.assign ──────────▶│ (→assigned, ownership alice→devin, AAP delegate)
  │                          │◀── (agent 执行)
  │                          │◀── checkpoint.raise ────│ (→blocked)
  │◀── checkpoint 通知 ──────│  (经 channel)
  │── checkpoint.resolve ───▶│ (choose opt_b → in_progress, AAP resume)
  │                          │◀── (继续执行)
  │                          │◀── artifact.commit ─────│ (v1, →review_ready)
  │◀── review 邀请 ──────────│
  │── review.submit ────────▶│ (changes_requested → in_progress, AAP resume)
  │                          │◀── (返工)
...
验证：21 操作足以表达完整闭环；ownership 流转 4 次全部入 audit；HACP↔AAP 衔接点干净。
