...
> 本文不是新的 Capability 协议规范。它是 HLP 如何记录既有能力生态中人类决策相关证据的路由说明。
...
HLP core 不要求感知能力。只有当人类决策、Artifact provenance 或 audit replay
需要说明能力来源时，HLP 才保存 opaque 外部证据引用，而不是感知 transport 或
invocation 细节。
...
|------|----------|------------------|
| MCP | 工具通过 MCP server 暴露 | 需要审计时将 MCP tool 映射为 `ExternalRef(kind="capability")`，transport 留在 host/agent 层。 |
...
| function calling | 能力来自模型 provider 的 function schema | 宿主侧补 discovery 和 stable id 后再给 HLP evidence ref。 |
