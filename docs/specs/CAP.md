...
- 谁是 caller（不区分人/agent）—— 那是 HLP 的事
- 会话与状态 —— 那是 loop1 的事
- 组织与权限模型 —— 那是 loop2 的事
...
这是 CAP 暴露给上层（AAP/HLP）的**唯一合法引用形式**。上层 **MUST** 只通过 CapabilityRef 引用能力，**MUST NOT** 感知底层 transport。
...
> 此对象在 HLP.md §5.3 和设计稿层间契约中作为唯一的跨层载体。
...
HLP 通过 `Constraints.must_use_capabilities` 引用能力，**只**用 CapabilityRef。HLP **MUST NOT** 直接调 CAP。
