...
> 本文不是新的 Agent-Agent 协议规范。它是 HLP 接入既有 agent 协议和 runtime 的路由说明。

## 1. 定位

HLP 只定义人机责任闭环。agent 间如何发现、委派、调度、执行，应该继续交给已有 L1 生态：
...
本文只说明：一个 HLP 实现接入这些 runtime 时，边界上必须保留哪些语义。

---

## 2. HLP 对 L1 的最小期望

| HLP 需求 | L1 runtime / adapter 需要做到 |
|----------|------------------------------|
...
| `ownership.transfer` | 如果执行权转移到另一个 agent，runtime handoff 必须保留原 TaskID。 |
...
|------|----------|------------------|
| A2A 风格 runtime | agent card / task / status update 已存在 | 把 HLP `Task.id` 放入 task metadata 或 extension field。 |
...
| 自研 runtime | 平台自己调度 agent | 暴露 delegate / block / resume / handoff / events 这几个 adapter 能力即可。 |
