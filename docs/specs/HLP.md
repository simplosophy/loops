...
HLP（Human Loop Protocol，人机责任闭环协议）定义**人**与**自主 agent** 如何围绕一个有边界的工作单元（**Task**）进行委派、把关、交付与治理。它是 [Loops Protocol Stack](../plans/2026-06-19-loops-protocol-stack.md) 的顶层协议，填补了 MCP（agent↔工具）和 A2A（agent↔agent）未覆盖的维度：**agent 与其负责人之间的责任闭环语义**。
...
HLP 是 Loops 栈的 L2。跨层通信 **MUST** 通过以下契约对象，**MUST NOT** 直接读写邻层内部状态。
...
验证：21 操作足以表达完整闭环；ownership 流转 4 次全部入 audit；HLP↔AAP 衔接点干净。
