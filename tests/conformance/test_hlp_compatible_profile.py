...
def test_hlp_compatible_profile_golden_23_operation_flow():
    ...
    interrupt = run(ops.task_interrupt(
        task.id,
        by="user_alice",
    ...
    deliverable_v1 = run(ops.artifact_commit(
        task_id=task.id,
        type="report",
    ...
    deliverable_v2 = run(ops.artifact_commit(
        task_id=task.id,
        type="report",
    ...
    expiring_checkpoint = run(ops.checkpoint_raise(
        task_id=expiring.id,
        kind="approval",
